...
describe("Dashboard page", () => {
  ...
  it("should display the top 10 movies of the genre the user clicks", async () => {
    ...
    try {
      ...
      for (const genre in TOP_10) {
        await driver.wait(
          until.elementLocated(By.id(`button-${genre}`)),
          10000
        );
  ...
  async function validateTopMovies(genre, result) {
    ...
    for (const i in result) {
      const title = await result[i]
        .findElement(By.className("title"))
        .getText();
      const rating = await result[i]
        .findElement(By.className("rating"))
        .getText();
      const votes = await result[i]
        .findElement(By.className("votes"))
        .getText();
...
describe("Best Genres page", () => {
  it("should return the correct movies for a given decade", async () => {
    let driver = await new webdriver.Builder()
      .withCapabilities(webdriver.Capabilities.chrome())
      .build();
    try {
      for (const decade of Object.keys(BEST_GENRES)) {
        await driver.get("http://localhost:3000/bestgenres");
        ...
        const submitButton = await driver.findElement(
          By.id("decadesSubmitBtn")
        );
        await submitButton.click();
        await driver.wait(
          until.elementLocated(By.className("movieResults")),
          60000
        );
  ...
  async function validateDecadeResults(decade, results) {
    ...
    for (const i in results) {
      const observedGenre = await results[i]
        .findElement(By.className("genre"))
        .getText();
      const observedRating = await results[i]
        .findElement(By.className("rating"))
        .getText();
